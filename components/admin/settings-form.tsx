...
export function SettingsForm({ initialSettings }: { initialSettings: SiteSettings }) {
  ...
  const updateTheme = (theme: "light" | "dark" | "contrast", key: "background" | "primary" | "secondary", value: string) =>
    setSettings(s => ({ ...s, appearance: { ...s.appearance, themes: { ...s.appearance.themes, [theme]: { ...s.appearance.themes[theme], [key]: value } } } }));
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); void save(); }}>
      ...
      <fieldset><legend>Appearance</legend>
        ...
        {(["light", "dark", "contrast"] as const).map(theme => (
          <fieldset key={theme}><legend>{theme === "contrast" ? "High contrast" : `${theme[0].toUpperCase()}${theme.slice(1)} theme`}</legend>
            {(["background", "primary", "secondary"] as const).map(key => (
              <label key={key}>{key}<input type="color" value={settings.appearance.themes[theme][key]} onChange={e => updateTheme(theme, key, e.target.value)} /></label>
            ))}
          </fieldset>
        ))}
      </fieldset>
