...
async function fetchPage(url: string): Promise<{ html: string; finalUrl: string }> {
  ...
  try {
    const response = await fetch(url, { redirect: "follow", signal: controller.signal, headers: { "user-agent": "ShortcodeGenMetadata/1.0" } });
...
export async function processMetadataJob(jobId: string) {
  ...
  try {
    ...
    await db.shortLink.update({ where: { id: job.shortLinkId }, data: {
      title: title(result.html), description, imageUrl: image, canonicalUrl: canonical,
      faviconUrl: icon, metadataSource: result.finalUrl, contentHash,
      lastCheckedAt: new Date(), lastSuccessfulFetchAt: new Date(),
    } });
...
export async function enqueueMetadataJob(shortLinkId: string) {
  return db.job.create({ data: { type: "METADATA", shortLinkId, status: "pending", runAfter: new Date() } });
}
